...
public class EdgeImplTest {
    ...
    public void testGetConnectedEdges(ObjectNode node) throws NoSuchMethodException {
        ...
        for (StudentMethodCall actual : results) {
            try {
                ...
                try {
                    assertTrue(Class.forName("java.util.Collections$UnmodifiableSet").isAssignableFrom(
                            actual.call.returnValue().getClass()) ||
                            actual.call.returnValue().getClass().getName().contains("java.util.ImmutableCollections"),
                            context,
                            r -> "getConnectedEdges() did not return immutable set! Returned Object of class " +
                                    actual.call.returnValue().getClass());
    ...
    private static Stream<Arguments> provideGetConnectedRails() {
        return Project_TestP.parseJsonFile("hProjekt/model/EdgeImpl_getConnectedRails.json");
    }
